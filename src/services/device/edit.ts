...
export default async ({ config_dev, context, scope, account, environment }: RouterConstructorDevice) => {
  ...
  const new_subgroup_id = (scope[0] as any)["tags.subgroup_id"];
  ...
  if (new_subgroup_id) {
    const { tags: current_device_tags } = await account.devices.info(dev_id);
    const new_device_tags = current_device_tags.filter((x) => x.key !== "subgroup_id");
    new_device_tags.push({ key: "subgroup_id", value: new_subgroup_id });
    await account.devices.edit(dev_id, { tags: new_device_tags });
  }
};
